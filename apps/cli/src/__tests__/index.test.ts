...
// pnpm --filter @ai-code-orchestrator/cli test src/__tests__/index.test.ts
...
function buildExtensionIfNeeded(): void {
	if (!isExtensionBuilt()) {
		execSync("pnpm --filter ai-code-orchestrator bundle", { cwd: findMonorepoRoot(), stdio: "inherit" })
