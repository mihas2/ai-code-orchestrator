/**
 * ExtensionHost - Loads and runs the AI Code Orchestrator extension in CLI mode
 *
...
} from "@ai-code-orchestrator/vscode-shim"
import { DebugLogger, setDebugLogEnabled } from "@ai-code-orchestrator/core/cli"
...
// When running from a release tarball, AICO_CLI_ROOT is set by the wrapper script.
...
const CLI_PACKAGE_ROOT = process.env.AICO_CLI_ROOT || findCliPackageRoot()
...
export class ExtensionHost extends EventEmitter implements ExtensionHostInterface {
	...
	private initialSettings: AiCodeOrchestratorSettings
	...
	constructor(options: ExtensionHostOptions) {
		...
		this.previousCliRuntimeEnv = process.env.AICO_CLI_RUNTIME
		...
		const baseSettings: AiCodeOrchestratorSettings = {
	...
	public markWebviewReady(): void {
		...
		setRuntimeConfigValues("ai-code-orchestrator", this.initialSettings as Record<string, unknown>)
	...
	async dispose(): Promise<void> {
		...
		if (this.previousCliRuntimeEnv === undefined) {
			...
			process.env.AICO_CLI_RUNTIME = this.previousCliRuntimeEnv
		}
	}
}
