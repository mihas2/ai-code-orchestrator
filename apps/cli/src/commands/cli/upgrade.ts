...
export function runUpgradeInstaller(version?: string, spawnImpl: typeof spawn = spawn): Promise<void> {
	return new Promise((resolve, reject) => {
		const env = version ? { ...process.env, AICO_VERSION: version } : process.env
...
export async function upgrade(options: UpgradeOptions = {}): Promise<void> {
	...
	if (compareVersions(latestVersion, currentVersion) <= 0) {
		console.log("AI Code Orchestrator CLI is already up to date.")
		return
	}

	console.log(`Upgrading AI Code Orchestrator CLI from ${currentVersion} to ${latestVersion}...`)
	if (runInstaller) {
		await runInstaller()
	} else {
