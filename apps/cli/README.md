# @AI Code Orchestrator/cli

Command Line Interface for AI Code Orchestrator - Run the AI Code Orchestrator agent from the terminal without VSCode.

## Overview

This CLI uses the `@AI Code Orchestrator/vscode-shim` package to provide a VSCode API compatibility layer, allowing the main AI Code Orchestrator extension to run in a Node.js environment.
...
```bash
AICO_INSTALL_DIR=/opt/ai-code-orchestrator AICO_BIN_DIR=/usr/local/bin curl -fsSL ... | sh
```
...
```bash
rm -rf ~/.ai-code-orchestrator/cli ~/.local/bin/aico
```
...
aico "What is this project?" -w ~/Documents/my-project
```

You can also run without a prompt and enter it interactively in TUI mode:

```bash
aico -w ~/Documents/my-project
```
...
```bash
aico "Refactor the utils.ts file" --require-approval -w ~/Documents/my-project
```
...
```bash
# Prompt is required
aico --print "Summarize this repository"

# Create a new task with a specific session ID (UUID)
aico --print --create-with-session-id 018f7fc8-7c96-7f7c-98aa-2ec4ff7f6d87 "Summarize this repository"
```
...
```bash
printf '{"command":"start","requestId":"1","prompt":"1+1=?"}\n' | aico --print --stdin-prompt-stream --output-format stream-json
...
2. **ExtensionHost** (`extension-host.ts`):
    ...
    - Creates a VSCode API mock using `@AI Code Orchestrator/vscode-shim`
