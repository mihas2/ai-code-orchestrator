...
# AI Code Orchestrator 3.11.9 Release Notes (2025-04-07)
...
## Custom AI Code Orchestrator Instructions (`.ai-code-orchestrator/` Directories)

- Introduced support for placing multiple instruction files within `.ai-code-orchestrator/rules/` (for workspace-wide) and `.ai-code-orchestrator/rules-{modeSlug}/` (for mode-specific) directories. AI Code Orchestrator recursively reads all files within these directories, appending them to the system prompt in **alphabetical order** by filename.
- This allows for better organization and management of complex instruction sets compared to single files.
- This directory-based method now takes precedence over the single `.aicorules` or `.aicorules-{modeSlug}` files if the corresponding directory exists and contains files. (Thanks upamune!)
...
- Added command to focus AI Code Orchestrator input field (thanks axkirillov!).
