---
description: AI Code Orchestrator 3.14 brings Gemini caching, Boomerang Orchestrator mode, improved file editing tools, and a new minimalist UI design.
keywords:
...
# AI Code Orchestrator 3.14 Combined
...
- Automatically close files opened by edit tools (`apply_diff`, `insert_content`, `search_and_replace`, `write_to_file`) after changes are approved. This prevents cluttering the editor with files opened by AI Code Orchestrator and helps clarify context by only showing files intentionally opened by the user.
...
- **Override Warning:** AI Code Orchestrator now displays a warning indicator in the chat input when a system prompt override is active for the current mode.
...
- Corrected word wrapping in AI Code Orchestrator message titles (thanks zhangtony239!).
