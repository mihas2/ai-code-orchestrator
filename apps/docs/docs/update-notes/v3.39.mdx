---
description: AI Code Orchestrator 3.39 includes task-level provider profile persistence, improved multimodal workflows, new provider/model options, and reliability fixes across tool calling.
keywords:
  - AI Code Orchestrator 3.39
  - new features
  - bug fixes
---

# AI Code Orchestrator 3.39 Release Notes (Combined)

AI Code Orchestrator 3.39 includes task-level provider profile persistence, improved multimodal workflows, new provider/model options, and reliability fixes across tool calling.

## Features

* **Task-level API configuration profiles**: AI Code Orchestrator can persist the selected provider profile per task so follow-up sessions keep using the same provider/model settings.
* **Image file @mentions**: You can mention images in your workspace to give the model visual context directly from chat.
* **Headless AI Code Orchestrator (CLI + VS Code shim)**: Run AI Code Orchestrator in a headless environment with a basic CLI, plus an installer and an option to use the CLI for evals.
* **Debug proxy routing**: Adds debug-mode proxy routing to help troubleshoot API calls.

## QOL Improvements

* **BRRR naming update**: Renames YOLO to BRRR.
* **Smarter file @mention search**: Filters file @mention results using `.aicoignore`.
...
* Renames AI Code Orchestrator Cloud Provider to **AI Code Orchestrator Router** for clearer branding.
* Updates AI Code Orchestrator Router service name and related wording so the UI uses consistent naming.
...
* Adds an interactive TUI mode to the AI Code Orchestrator CLI.

### 3.39.2 (2026-01-09)

* Adds an **Enable debug mode** toggle in AI Code Orchestrator settings (About AI Code Orchestrator) so you can turn debug logging on/off from the extension UI.
...
* Updates AI Code Orchestrator CLI defaults by switching the default model to `anthropic/claude-opus-4.5`.
* Updates AI Code Orchestrator Cloud’s Terms of Service (effective 2026-01-09).
