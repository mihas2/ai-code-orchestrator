---
description: AI Code Orchestrator 2.2.23 corrects context window size information for gemini-2.0-flash-thinking-exp-1219 model for accurate usage.
keywords:
    - AI Code Orchestrator
    - v2.2.23
    - release notes
    - gemini
    - context window
    - bug fix
    - model configuration
---

# AI Code Orchestrator 2.2.23 Release Notes

This patch release fixes context window information for a specific Gemini model.

---

## Fixes

- Corrected the context window size information for the `gemini-2.0-flash-thinking-exp-1219` model. (thanks student20880!)
