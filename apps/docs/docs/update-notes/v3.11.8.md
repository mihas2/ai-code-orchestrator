...
# AI Code Orchestrator 3.11.8 Release Notes (2025-04-05)

This patch release includes performance improvements and UI updates.

---

## Introduction of `.aicorules` Files

- Added support for `.aicorules` files (e.g., `.aicorules` for workspace-wide, `.aicorules-code` for mode-specific) as a way to manage custom instructions directly within your project. This provides an alternative to defining instructions solely within the extension's settings UI. A deprecation warning for the older `.clinerules` file was also added. (Thanks upamune!)
