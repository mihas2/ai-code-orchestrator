---
description: Learn how to create reusable skills that package task-specific instructions, making AI Code Orchestrator more effective at specialized workflows.
keywords:
  - skills
...
Skills package task-specific instructions that AI Code Orchestrator loads on-demand when your request matches the skill's purpose. Unlike custom instructions that apply to everything, skills activate only when needed—making AI Code Orchestrator more effective at specialized tasks without cluttering the base prompt.
...
**Skills solve this**: Create a skill for PDF processing, and AI Code Orchestrator only loads those instructions when you actually ask to work with PDFs. This keeps the system prompt focused and gives AI Code Orchestrator deep expertise in specific domains without affecting unrelated tasks.
...
- **Team Sharing**: Version-control project skills in `.ai-code-orchestrator/skills/` for consistent team workflows
- **Personal Library**: Build a global skills library in `~/.ai-code-orchestrator/skills/` that works across all projects
...
**Level 1: Discovery** - AI Code Orchestrator reads each `SKILL.md` file and parses its frontmatter to extract `name` and `description`. Only this metadata is stored for matching—the full content isn't held in memory until needed.

**Level 2: Instructions** - When your request matches a skill's description, AI Code Orchestrator uses [`read_file`](/advanced-usage/available-tools/read-file) to load the full SKILL.md instructions into context.

**Level 3: Resources** - The prompt tells AI Code Orchestrator it may access bundled files (scripts, templates, references) alongside the skill. There's no separate resource manifest—AI Code Orchestrator discovers these files on-demand when the instructions reference them.

This architecture means skills remain dormant until activated—they don't bloat your base prompt. You can install many skills, and AI Code Orchestrator loads only what's relevant for each task.
...
```bash
# Linux/macOS — AI Code Orchestrator-specific (higher priority)
~/.ai-code-orchestrator/skills/{skill-name}/SKILL.md

# Linux/macOS — cross-agent (shared with other agent tools)
~/.agents/skills/{skill-name}/SKILL.md

# Windows
%USERPROFILE%\.ai-code-orchestrator\skills\{skill-name}\SKILL.md
%USERPROFILE%\.agents\skills\{skill-name}\SKILL.md
```

**Project skills** (specific to current workspace):
```bash
# AI Code Orchestrator-specific (higher priority)
<project-root>/.ai-code-orchestrator/skills/{skill-name}/SKILL.md
...
mkdir -p ~/.ai-code-orchestrator/skills/pdf-processing
touch ~/.ai-code-orchestrator/skills/pdf-processing/SKILL.md
```
...
- The description tells AI Code Orchestrator when to use this skill—be specific

#### 4. Test the skill

Ask AI Code Orchestrator something matching the description:
```
"Can you help me extract tables from this PDF file?"
```

AI Code Orchestrator should recognize the request matches your skill description, load the SKILL.md file, and follow its instructions.
...
```
~/.ai-code-orchestrator/skills/                    # Global AI Code Orchestrator-specific skills (high priority)
├── pdf-processing/
│   ├── SKILL.md                 # Required
...
.ai-code-orchestrator/skills/                      # Project AI Code Orchestrator-specific skills (override global)
└── custom-pdf-workflow/
    └── SKILL.md
...
```
~/.ai-code-orchestrator/skills-code/              # Only in Code mode (AI Code Orchestrator-specific, global)
└── refactoring-patterns/
    └── SKILL.md

.ai-code-orchestrator/skills-architect/           # Only in Architect mode (AI Code Orchestrator-specific, project)
...
When skills with the same name exist in multiple locations, this priority applies (highest to lowest). `.ai-code-orchestrator/` paths always take precedence over `.agents/` paths at the same project level:

1. **Project `.ai-code-orchestrator` mode-specific** (`.ai-code-orchestrator/skills-code/my-skill/`) — highest priority
2. **Project `.ai-code-orchestrator` generic** (`.ai-code-orchestrator/skills/my-skill/`)
3. **Project `.agents` mode-specific** (`.agents/skills-code/my-skill/`)
4. **Project `.agents` generic** (`.agents/skills/my-skill/`)
5. **Global `.ai-code-orchestrator` mode-specific** (`~/.ai-code-orchestrator/skills-code/my-skill/`)
6. **Global `.ai-code-orchestrator` generic** (`~/.ai-code-orchestrator/skills/my-skill/`)
7. **Global `.agents` mode-specific** (`~/.agents/skills-code/my-skill/`)
8. **Global `.agents` generic** (`~/.agents/skills/my-skill/`) — lowest priority

A **project** skill always overrides a **global** skill of the same name. Within the same project level, `.ai-code-orchestrator/` overrides `.agents/`.
...
AI Code Orchestrator automatically discovers skills:
...
ln -s /shared/company-skills ~/.ai-code-orchestrator/skills/company-standards
```
...
**Symptom**: AI Code Orchestrator doesn't use your skill even when you request something matching the description.
...
4. **Description too vague**: Make descriptions specific so AI Code Orchestrator can match them to requests
   ```markdown
   # ✗ Vague
...
**Symptom**: AI Code Orchestrator reads the skill but doesn't follow instructions.
...
**Symptom**: Unclear which skill AI Code Orchestrator will use when multiple might match.
...
**Solution**: Place skills in `.ai-code-orchestrator/skills/` within your project and commit to version control. Each team member gets the same skills automatically.
...
AI Code Orchestrator skills follow the [Agent Skills](https://agentskills.io/) format for skill packaging and metadata. Skills are **instruction packages with optional bundled files**—they don't register new executable tools.
...
#### AI Code Orchestrator-specific enhancements

AI Code Orchestrator adds mode-specific targeting and cross-agent compatibility beyond the base format:

- **AI Code Orchestrator-specific locations**: `.ai-code-orchestrator/skills/`, `~/.ai-code-orchestrator/skills/` (higher priority)
- **Cross-agent locations**: `.agents/skills/`, `~/.agents/skills/` (shared with other agent tools)
- **Mode-specific directories**: `skills-{mode}/` (e.g., `skills-code/`, `skills-architect/`) in both `.ai-code-orchestrator/` and `.agents/` paths enable mode targeting
