---
description: Discover and install MCP servers and custom modes from the AI Code Orchestrator Marketplace to extend your AI coding assistant's capabilities.
keywords:
...
The AI Code Orchestrator Marketplace is a central hub for discovering and installing extensions, known as MCPs (Model Context Protocol) and Modes. It allows you to easily extend the functionality of AI Code Orchestrator to fit your specific needs and workflows.
...
The AI Code Orchestrator Marketplace is available directly within the AI Code Orchestrator extension in VS Code. Access it by clicking the marketplace icon <Codicon name="extensions" /> in the top menu bar.
...
MCPs allow AI Code Orchestrator to connect to and interact with various AI models, APIs, and other external tools. By installing an MCP, you can configure AI Code Orchestrator to use different language models (like those from OpenAI, Anthropic, or others) or to integrate with other services. Learn more about [What is MCP?](/features/mcp/what-is-mcp) and [how to use MCP in AI Code Orchestrator](/features/mcp/using-mcp-in-aico).

<img src="/img/marketplace/marketplace-1.png" alt="MCP tab showing available MCPs with install and remove buttons" width="500" />

### Modes
Modes are custom sets of instructions and rules that tailor AI Code Orchestrator's behavior for specific tasks. For example, you might find a "React Component" mode that is optimized for creating React components, or a "Documentation Writer" mode for writing technical documentation. Learn more about [using modes](/basic-usage/using-modes) and [creating custom modes](/features/custom-modes).
...
- **Configuration File**:
    - **MCPs**: [`.ai-code-orchestrator/mcp.json`](/features/mcp/using-mcp-in-aico#configuring-mcp-servers) in the root of your project.
    - **Modes**: [`.agent-modes`](/features/custom-modes#3-manual-configuration-yaml--json) in the root of your project.
...
- **Configuration File**:
    - **MCPs**: [`mcp_settings.json`](/features/mcp/using-mcp-in-aico#configuring-mcp-servers) in the AI Code Orchestrator extension's global settings directory.
    - **Modes**: [`custom_modes.yaml`](/features/custom-modes) in the AI Code Orchestrator extension's global settings directory.
...
5. AI Code Orchestrator automatically adds the item to the appropriate configuration file. If the file doesn't exist, AI Code Orchestrator will create it for you. The file is then opened for your review.
...
4. AI Code Orchestrator removes the item from the corresponding configuration file.
...
### Installation Errors
- **Invalid YAML/JSON**: The configuration file (`.agent-modes`, `.ai-code-orchestrator/mcp.json`, etc.) has a syntax error. To prevent data loss, AI Code Orchestrator will not modify a corrupted configuration file. Please fix the syntax error before installing or removing items.
- **File Not Found**: Rare error - AI Code Orchestrator automatically creates necessary configuration files.
...
4. **Check AI Code Orchestrator logs**: Look for error messages in the AI Code Orchestrator output panel.
...
- [Using MCP in AI Code Orchestrator](/features/mcp/using-mcp-in-aico) - Detailed configuration and usage guide
