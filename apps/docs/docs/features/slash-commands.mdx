---
description: Learn how to create and use custom slash commands in AI Code Orchestrator to automate workflows and extend functionality with markdown-based definitions.
keywords:
  - slash commands
  - custom commands
  - AI Code Orchestrator commands
  - command automation
  - workflow automation
  - markdown commands
  - .ai-code-orchestrator/commands
...
Create custom slash commands to automate repetitive tasks and extend AI Code Orchestrator's functionality with simple markdown files.

:::tip Quick Start
Type `/` in chat to select a command. To create or manage commands, open Settings > Slash Commands. You can still store commands in `.ai-code-orchestrator/commands/` (project) or `~/.ai-code-orchestrator/commands/` (global).
:::
...
Custom commands extend AI Code Orchestrator's functionality by adding markdown files to specific directories:

- **Project-specific**: `.ai-code-orchestrator/commands/` in your workspace root
- **Global**: `~/.ai-code-orchestrator/commands/` in your home directory
...
- **`mode`**: (Optional) Mode slug to switch to before running the command (e.g., `code`, `architect`). AI Code Orchestrator switches to this mode first, then executes the command content in that mode's context
...
1. Click the gear icon in AI Code Orchestrator and open Settings
...
When you run this command, AI Code Orchestrator switches to Architect mode first, then processes the command content with Architect's role definition and tool restrictions.
...
AI Code Orchestrator includes powerful built-in commands that provide specialized functionality:
...
- Generates mode-specific `AGENTS.md` files in `.ai-code-orchestrator/rules-*` directories
...
- **Check file location**: Ensure custom command files are in `.ai-code-orchestrator/commands/` or `~/.ai-code-orchestrator/commands/`
...
**Command Conflicts:**
- Project commands (`.ai-code-orchestrator/commands/`) override global commands (`~/.ai-code-orchestrator/commands/`) with the same name
...
**File System Errors:**
- **Permission Issues**: Ensure you have write permissions to the `.ai-code-orchestrator/commands/` directory
...
- [Using Modes](/basic-usage/using-modes) - Learn about AI Code Orchestrator's different operational modes
- [Custom Instructions](/features/custom-instructions) - Set persistent instructions for AI Code Orchestrator
