---
description: Learn how to use checkpoints in AI Code Orchestrator to save your progress, revert changes, and manage your coding workflow with confidence.
keywords:
  - checkpoints
  - save progress
  - version control
...
Checkpoints automatically version your workspace files during AI Code Orchestrator tasks, enabling non-destructive exploration of AI suggestions and easy recovery from unwanted changes.
...
Access checkpoint settings in AI Code Orchestrator settings under the "Checkpoints" section:
...
2. Configure checkpoint behavior:
   ...
   <img src="/img/checkpoints/checkpoints.png" alt="Checkpoint settings in AI Code Orchestrator configuration" width="500" />
...
- **Scope**: Checkpoints only capture changes made during active AI Code Orchestrator tasks
...
When restoration executes, AI Code Orchestrator:
...
The `.aicoignore` file (which controls AI access to files) is separate from checkpoint tracking:
- Files excluded by `.aicoignore` but not by `.gitignore` will still be checkpointed
- Changes to AI-inaccessible files can still be restored through checkpoints

This separation is intentional, as `.aicoignore` limits which files the AI can access, not which files should be tracked for version history.
