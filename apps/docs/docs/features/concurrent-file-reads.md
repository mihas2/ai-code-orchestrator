---
description: Learn how AI Code Orchestrator's Concurrent File Reads feature speeds up development by reading up to 100 files simultaneously for better context understanding.
keywords:
...
The Concurrent File Reads feature allows AI Code Orchestrator to read multiple files from your workspace in a single step. This significantly improves efficiency when working on tasks that require context from several files, as AI Code Orchestrator can gather all the necessary information at once instead of reading files one by one.
...
- **Increased Speed**: Reduces the time it takes for AI Code Orchestrator to understand your code by minimizing the number of back-and-forth steps.
- **Better Context**: Allows AI Code Orchestrator to build a more complete mental model of your code, leading to more accurate and relevant responses.
...
**Faster Context Building**: Previously, when AI Code Orchestrator needed to understand your project, you'd see multiple requests like:

- "Can I read `src/app.js`?" → You approve
- "Now can I read `src/utils.js`?" → You approve
- "And can I read `src/config.json`?" → You approve

**With concurrent file reads**: AI Code Orchestrator asks once to read all related files together, getting the full picture immediately and providing better assistance faster.

---

## How it Works

When you ask AI Code Orchestrator to perform a task that involves multiple files, it will automatically identify the relevant files and read them together. This is especially useful for:
...
AI Code Orchestrator is instructed to use this feature efficiently by prioritizing the most critical files and reading them in a single batch. The [`read_file`](/advanced-usage/available-tools/read-file) tool automatically accepts multiple files in a single request.

When AI Code Orchestrator requests to read multiple files, you'll see a batch approval interface that displays:
