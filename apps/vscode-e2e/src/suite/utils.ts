import { AiCodeOrchestratorEventName, type AiCodeOrchestratorAPI } from "@ai-code-orchestrator/types"
...
type WaitUntilAbortedOptions = WaitForOptions & {
	api: AiCodeOrchestratorAPI
	taskId: string
}

export const waitUntilAborted = async ({ api, taskId, ...options }: WaitUntilAbortedOptions) => {
	const set = new Set<string>()
	api.on(AiCodeOrchestratorEventName.TaskAborted, (taskId) => set.add(taskId))
...
export const waitUntilCompleted = async ({ api, taskId, ...options }: WaitUntilCompletedOptions) => {
	const set = new Set<string>()
	api.on(AiCodeOrchestratorEventName.TaskCompleted, (taskId) => set.add(taskId))
	await waitFor(() => set.has(taskId), options)
}
