...
import { AiCodeOrchestratorEventName, type ClineMessage } from "@ai-code-orchestrator/types"

import { waitFor, sleep } from "../utils"
import { setDefaultSuiteTimeout } from "../test-utils"

suite.skip("AI Code Orchestrator use_mcp_tool Tool", function () {
	...
	suiteSetup(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aico-test-mcp-"))
		...
		testFiles = {
			...
			mcpConfig: path.join(workspaceDir, ".ai-code-orchestrator", "mcp.json"),
		}
		...
		// Create .ai-code-orchestrator directory and MCP configuration file
		const aicoDir = path.join(workspaceDir, ".ai-code-orchestrator")
	...
	suiteTeardown(async () => {
		...
		// Clean up .ai-code-orchestrator directory
		const workspaceDir = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || tempDir
		const aicoDir = path.join(workspaceDir, ".ai-code-orchestrator")
		try {
			await fs.rm(aicoDir, { recursive: true, force: true })
		} catch {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		api.on(AiCodeOrchestratorEventName.Message, messageHandler)
		...
		api.on(AiCodeOrchestratorEventName.TaskStarted, taskStartedHandler)
		...
		api.on(AiCodeOrchestratorEventName.TaskCompleted, taskCompletedHandler)
		await sleep(2000) // Wait for AI Code Orchestrator to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this AI Code Orchestrator won't load the MCP server
		...
		try {
			...
			// Clean up
			api.off(AiCodeOrchestratorEventName.Message, messageHandler)
			api.off(AiCodeOrchestratorEventName.TaskStarted, taskStartedHandler)
			api.off(AiCodeOrchestratorEventName.TaskCompleted, taskCompletedHandler)
		}
	})
	...
	test("Should request MCP filesystem list_directory tool and complete successfully", async function () {
		...
		try {
			...
			const hasAicoDir = responseText.includes(".ai-code-orchestrator")

			// At least one of our test files or the .ai-code-orchestrator directory should be present
			assert.ok(
				hasTestFile || hasDataFile || hasAicoDir,
				`MCP server response should contain our test files or .ai-code-orchestrator directory. Expected to find: '${path.basename(testFiles.simple)}', '${path.basename(testFiles.testData)}', or '.ai-code-orchestrator'. Got: ${responseText.substring(0, 200)}...`,
			)
	...
	test.skip("Should request MCP filesystem directory_tree tool and complete successfully", async function () {
		...
		try {
			...
			const hasTestFiles =
				...
				responseText.includes(".ai-code-orchestrator") ||
				responseText.includes(".txt") ||
				responseText.includes(".json") ||
