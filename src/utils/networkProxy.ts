 ...
 *
 * Provides proxy configuration for all outbound HTTP/HTTPS requests from the AI Code Orchestrator extension.
...
async function configureUndiciProxy(config: ProxyConfig): Promise<void> {
	...
	try {
		...
		// To ensure AI Code Orchestrator's `fetch()` calls are proxied, patch global fetch in debug mode.
