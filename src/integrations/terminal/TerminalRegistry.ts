...
import { AicoTerminal, AicoTerminalProvider } from "./types"
...
export class TerminalRegistry {
	private static terminals: AicoTerminal[] = []
		...
		try {
			const startDisposable = vscode.window.onDidStartTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionStartEvent) => {
					...
					if (terminal) {
						...
						console.error(
							"[onDidStartTerminalShellExecution] Shell execution started, but not from a AI Code Orchestrator-registered terminal:",
							e,
						)
					}
				},
			...
			const endDisposable = vscode.window.onDidEndTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionEndEvent) => {
					...
					if (!terminal) {
						console.error(
							"[onDidEndTerminalShellExecution] Shell execution ended, but not from a AI Code Orchestrator-registered terminal:",
							e,
						)
	...
	public static createTerminal(cwd: string, provider: AicoTerminalProvider): AicoTerminal {
		let newTerminal
	...
	public static async getOrCreateTerminal(
		cwd: string,
		taskId?: string,
		provider: AicoTerminalProvider = "vscode",
	): Promise<AicoTerminal> {
		const terminals = this.getAllTerminals()
		let terminal: AicoTerminal | undefined
	 ...
	 */
	public static getTerminals(busy: boolean, taskId?: string): AicoTerminal[] {
	 ...
	 * @returns Array of Terminal objects
	 */
	public static getBackgroundTerminals(busy?: boolean): AicoTerminal[] {
		return this.getAllTerminals().filter((t) => {
	...
	private static getAllTerminals(): AicoTerminal[] {
		this.terminals = this.terminals.filter((t) => !t.isClosed())
		return this.terminals
	}

	private static getTerminalById(id: number): AicoTerminal | undefined {
	 ...
	 */
	private static getTerminalByVSCETerminal(vsceTerminal: vscode.Terminal): AicoTerminal | undefined {
