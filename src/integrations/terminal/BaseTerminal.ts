...
import type {
	AicoTerminalProvider,
	AicoTerminal,
	AicoTerminalCallbacks,
	AicoTerminalProcess,
	AicoTerminalProcessResultPromise,
	ExitCodeDetails,
} from "./types"

export abstract class BaseTerminal implements AicoTerminal {
	public readonly provider: AicoTerminalProvider
	...
	public process?: AicoTerminalProcess
	public completedProcesses: AicoTerminalProcess[] = []

	constructor(provider: AicoTerminalProvider, id: number, cwd: string) {
	...
	abstract runCommand(command: string, callbacks: AicoTerminalCallbacks): AicoTerminalProcessResultPromise
	...
	public setActiveStream(stream: AsyncIterable<string> | undefined, pid?: number): void {
		if (stream) {
			if (!this.process) {
				...
				console.warn(
					`[Terminal ${this.provider}/${this.id}] process is undefined, so cannot set terminal stream (probably user-initiated non-AI Code Orchestrator command)`,
				)
	 ...
	 */
	public getProcessesWithOutput(): AicoTerminalProcess[] {
