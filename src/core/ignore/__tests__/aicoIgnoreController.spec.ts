// npx vitest core/ignore/__tests__/AicoIgnoreController.spec.ts

import type { Mock } from "vitest"

import { AicoIgnoreController, LOCK_TEXT_SYMBOL } from "../AicoIgnoreController"
...
describe("AicoIgnoreController", () => {
	const TEST_CWD = "/test/path"
	let controller: AicoIgnoreController
	...
	beforeEach(() => {
		...
		// Create controller
		controller = new AicoIgnoreController(TEST_CWD)
	})

	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .aicoignore exists
		 */
		it("should load .aicoignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .aicoignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".aicoignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".aicoignore"), "utf8")

			// Verify content was stored
			expect(controller.aicoIgnoreContent).toBe("node_modules\n.git\nsecrets.json")
		...
		/**
		 * Tests the controller behavior when .aicoignore doesn't exist
		 */
		it("should allow all access when .aicoignore doesn't exist", async () => {
			// Setup mocks to simulate missing .aicoignore file
			...
			expect(controller.aicoIgnoreContent).toBeUndefined()
		 ...
		 */
		it("should set up file watcher for .aicoignore changes", async () => {
		 ...
		 */
		it("should handle errors when loading .aicoignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .aicoignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .aicoignore exists
		 */
		it("should allow all access when no .aicoignore content", async () => {
			// Create a new controller with no .aicoignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new AicoIgnoreController(TEST_CWD)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .aicoignore exists
		 */
		it("should allow all commands when no .aicoignore exists", async () => {
			// Create a new controller with no .aicoignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new AicoIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .aicoignore
		 */
		it("should generate formatted instructions when .aicoignore exists", async () => {
			// Setup .aicoignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .aicoignore exists
		 */
		it("should return undefined when no .aicoignore exists", async () => {
			// Setup no .aicoignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .aicoignore is created
		 */
		it("should reload .aicoignore when file is created", async () => {
			// Setup initial state without .aicoignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .aicoignore
			controller = new AicoIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .aicoignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .aicoignore is changed
		 */
		it("should reload .aicoignore when file is changed", async () => {
			// Setup initial state with .aicoignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .aicoignore is deleted
		 */
		it("should reset when .aicoignore is deleted", async () => {
			// Setup initial state with .aicoignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
