...
export interface ResolveImageMentionsOptions {
	...
	images?: string[]
	cwd: string
	aicoIgnoreController?: { validateAccess: (filePath: string) => boolean }
 ...
 * - Respects `.aicoignore` via `aicoIgnoreController.validateAccess` when provided
 */
export async function resolveImageMentions({
	...
	for (const mention of imageMentions) {
		...
		if (aicoIgnoreController && !aicoIgnoreController.validateAccess(relPath)) {
			continue
		}
