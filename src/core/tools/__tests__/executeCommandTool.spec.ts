...
import type { ToolUsage } from "@ai-code-orchestrator/types"
...
describe("executeCommandTool", () => {
	...
	const originalCliRuntime = process.env.AICO_CLI_RUNTIME
	...
	afterEach(() => {
		process.env.AICO_CLI_RUNTIME = originalCliRuntime
	})
	...
	describe("Error handling", () => {
		...
		it("should handle aicoignore validation failures", async () => {
			// Setup
			...
			const mockAicoIgnoreError = "AicoIgnore error"
			;(formatResponse.aicoIgnoreError as any).mockReturnValue(mockAicoIgnoreError)
			...
			expect(mockCline.say).toHaveBeenCalledWith("aicoignore_error", ".env")
			expect(formatResponse.aicoIgnoreError).toHaveBeenCalledWith(".env")
			expect(mockPushToolResult).toHaveBeenCalledWith(mockAicoIgnoreError)
			expect(mockAskApproval).not.toHaveBeenCalled()
			// executeCommandInTerminal should not be called since aicoignore blocked it
		})
	})
