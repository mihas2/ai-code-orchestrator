...
vi.mock("../../prompts/responses", () => ({
	formatResponse: {
		...
		aicoIgnoreError: vi.fn((path) => `Access denied: ${path}`),
...
describe("writeToFileTool", () => {
	...
	async function executeWriteFileTool(
		...
		mockCline.aicoIgnoreController.validateAccess.mockReturnValue(accessAllowed)
	...
	describe("access control", () => {
		it("validates and allows access when aicoIgnoreController permits", async () => {
			await executeWriteFileTool({}, { accessAllowed: true })

			expect(mockCline.aicoIgnoreController.validateAccess).toHaveBeenCalledWith(testFilePath)
	...
	describe("directory creation for new files", () => {
		...
		it.skipIf(process.platform === "win32")(
			"creates parent directories on the first partial with a known path",
			async () => {
	...
	describe("file operations", () => {
		it("successfully creates new files with full workflow", async () => {
			...
			expect(mockCline.fileContextTracker.trackFileContext).toHaveBeenCalledWith(testFilePath, "aico_edited")
	...
	describe("partial block handling", () => {
		...
		it("streams content feedback before editor updates", async () => {
			await executeWriteFileTool({}, { isPartial: true })
			const [kind, payload, partial, progress] = mockCline.ask.mock.calls[0]
			expect(kind).toBe("tool")
			expect(JSON.parse(payload).content).toBe(testContent)
			expect(partial).toBe(true)
			expect(progress).toEqual(expect.objectContaining({ text: expect.any(String) }))
			expect(mockCline.diffViewProvider.open).toHaveBeenCalledWith(testFilePath)
	...
	describe("error handling", () => {
		...
		it("handles partial streaming errors after path stabilizes", async () => {
			...
			// A known path is handled immediately, so the mocked editor error occurs on the first partial.
			await executeWriteFileTool({}, { isPartial: true })
