...
vi.mock("../../prompts/responses", () => ({
	formatResponse: {
		...
		aicoIgnoreError: vi.fn(
			(filePath: string) =>
				`Access to ${filePath} is blocked by the .aicoignore file settings. You must try to continue in the task without using this file, or ask the user to update the .aicoignore file.`,
		),
...
interface MockTaskOptions {
	...
	aicoIgnoreAllowed?: boolean
	maxImageFileSize?: number
	maxTotalImageSize?: number
}

function createMockTask(options: MockTaskOptions = {}) {
	const { supportsImages = false, aicoIgnoreAllowed = true, maxImageFileSize = 5, maxTotalImageSize = 20 } = options
	...
	return {
		...
		aicoIgnoreController: {
			validateAccess: vi.fn().mockReturnValue(aicoIgnoreAllowed),
		},
		fileContextTracker: {
...
describe("ReadFileTool", () => {
	...
	describe("AicoIgnore handling", () => {
		it("should block access to aicoignore-protected files", async () => {
			const mockTask = createMockTask({ aicoIgnoreAllowed: false })
			const callbacks = createMockCallbacks()

			await readFileTool.execute({ path: "secret.env" }, mockTask as any, callbacks)

			expect(mockTask.say).toHaveBeenCalledWith("aicoignore_error", "secret.env")
			expect(formatResponse.aicoIgnoreError).toHaveBeenCalledWith("secret.env")
			expect(callbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("blocked by the .aicoignore"))
		})
	})
