...
export type LeadAcceptance = TaskAcceptanceDecision & { attempt: number }
...
const sessionSchema = z.object({
	...
	acceptance: taskAcceptanceDecisionSchema.extend({ attempt: z.number().int().nonnegative() }).optional(),
	ambiguousRequest: z
...
export class VersionedLeadSessionStore implements LeadSessionStore {
	...
	async load(key: string) {
		...
		return parsed.success ? (parsed.data as LeadSession) : undefined
	}
...
export function acceptanceGate(
	...
	if (session.phase === "canceled")
		return {
			...
			outcome: "accepted",
			confidence: 1,
			rationale: "Canceled task does not require acceptance",
			criteria: decisions,
			attempt,
		}
