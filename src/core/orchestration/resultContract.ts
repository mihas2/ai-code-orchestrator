...
export const RESULT_CONTRACT_INSTRUCTION = `Your final attempt_completion response MUST end with one JSON ResultContract object (plain or fenced JSON) with exactly these fields: contractVersion=1; status (completed|partial|failed); summary; filesRead; filesChanged; artifactRefs; tests [{command,passed,exitCode?,outputRef?}]; assumptions; risks; openQuestions; nextActions. Report workspace-relative paths only and only within the declared file scopes. Do not include credentials, tokens, passwords, or secret values. Missing or invalid JSON makes the worker result fail.`
