...
const AGENT_MODES_FILENAME = ".agent-modes"
...
export class CustomModesManager {
	...
	private async getWorkspaceAgentModes(): Promise<string | undefined> {
		...
		const agentModesPath = path.join(workspaceRoot, AGENT_MODES_FILENAME)
	...
	private parseYamlSafely(content: string, filePath: string): any {
		...
		try {
			...
			// For .agent-modes files, try JSON as fallback
			if (filePath.endsWith(AGENT_MODES_FILENAME)) {
				try {
	...
	private async loadModesFromFile(filePath: string): Promise<ModeConfig[]> {
		try {
			...
			if (!result.success) {
				...
				// Show user-friendly error for .agent-modes files
				if (filePath.endsWith(AGENT_MODES_FILENAME)) {
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		const handleSettingsChange = async () => {
			try {
				...
				// Get modes from .agent-modes if it exists (takes precedence)
		...
		// Watch .agent-modes file - watch the path even if it doesn't exist yet
		const workspaceFolders = vscode.workspace.workspaceFolders
		if (workspaceFolders && workspaceFolders.length > 0) {
			const workspaceRoot = getWorkspacePath()
			const agentModesPath = path.join(workspaceRoot, AGENT_MODES_FILENAME)
			...
			const handleAgentModesChange = async () => {
				try {
					...
					const agentModes = await this.loadModesFromFile(agentModesPath)
					// .agent-modes takes precedence
					const mergedModes = await this.mergeCustomModes(agentModes, settingsModes)
					await this.context.globalState.update("customModes", mergedModes)
					this.clearCache()
					await this.onUpdate()
				} catch (error) {
					console.error(`[CustomModesManager] Error handling .agent-modes file change:`, error)
				}
			}
			...
			this.disposables.push(
				agentModesWatcher.onDidDelete(async () => {
					// When .agent-modes is deleted, refresh with only settings modes
					try {
						const settingsModes = await this.loadModesFromFile(settingsPath)
						...
						console.error(`[CustomModesManager] Error handling .agent-modes file deletion:`, error)
					}
				}),
			)
	...
	public async getCustomModes(): Promise<ModeConfig[]> {
		...
		// Get modes from .agent-modes if it exists.
		const agentModesPath = await this.getWorkspaceAgentModes()
		const agentModes = agentModesPath ? await this.loadModesFromFile(agentModesPath) : []
		...
		const mergedModes = [
			...agentModes.map((mode) => ({ ...mode, source: "project" as const })),
			...settingsModes
	...
	public async updateCustomMode(slug: string, config: ModeConfig): Promise<void> {
		try {
			...
			if (isProjectMode) {
				...
				const workspaceRoot = getWorkspacePath()
				targetPath = path.join(workspaceRoot, AGENT_MODES_FILENAME)
				const exists = await fileExistsAtPath(targetPath)

				logger.info(`${exists ? "Updating" : "Creating"} project mode in ${AGENT_MODES_FILENAME}`, {
					slug,
	...
	public async deleteCustomMode(slug: string): Promise<void> {
		try {
			...
			const projectMode = agentModes.find((m) => m.slug === slug)
	...
	private async deleteRulesFolder(slug: string, mode: ModeConfig): Promise<void> {
		try {
			...
			if (scope === "project") {
				...
				if (workspacePath) {
					rulesFolderPath = path.join(workspacePath, ".ai-code-orchestrator", `rules-${slug}`)
				} else {
				...
				rulesFolderPath = path.join(homeDir, ".ai-code-orchestrator", `rules-${slug}`)
			}
	...
	/**
	 * Checks if a mode has associated rules files in the .ai-code-orchestrator/rules-{slug}/ directory
	...
	public async checkRulesDirectoryHasContent(slug: string): Promise<boolean> {
		try {
			...
			if (!mode) {
				// If not in custom modes, check if it's in .agent-modes (project-specific)
				const workspacePath = getWorkspacePath()
				if (!workspacePath) {
					return false
				}

				const agentModesPath = path.join(workspacePath, AGENT_MODES_FILENAME)
				try {
					...
					if (agentModesExists) {
						...
						const agentModes = agentModesData?.customModes || []

						// Check if this specific mode exists in .agent-modes
						const modeInAgentModes = agentModes.find((m: any) => m.slug === slug)
						if (!modeInAgentModes) {
							return false // Mode not found anywhere
						}
					} else {
						return false // No .agent-modes file and not in custom modes
					}
				} catch (error) {
					return false // Cannot read .agent-modes and not in custom modes
				}
			}
			...
			if (isGlobalMode) {
				// For global modes, check in global .ai-code-orchestrator directory
				const globalAicoDir = getGlobalAicoDirectory()
				modeRulesDir = path.join(globalAicoDir, `rules-${slug}`)
			} else {
				// For project modes, check in workspace .ai-code-orchestrator directory
				const workspacePath = getWorkspacePath()
				if (!workspacePath) {
					return false
				}
				modeRulesDir = path.join(workspacePath, ".ai-code-orchestrator", `rules-${slug}`)
			}
	...
	public async exportModeWithRules(slug: string, customPrompts?: PromptComponent): Promise<ExportResult> {
		try {
				...
				if (workspacePath) {
					...
					try {
						...
						if (agentModesExists) {
							...
							// Find the mode in .agent-modes
							mode = agentModes.find((m: any) => m.slug === slug)
						}
					} catch (error) {
			...
			let baseDir: string
			if (isGlobalMode) {
				// For global modes, use the global .ai-code-orchestrator directory
				baseDir = getGlobalAicoDirectory()
			} else {
			...
			// Check for .ai-code-orchestrator/rules-{slug}/ directory (or rules-{slug}/ for global)
			const modeRulesDir = isGlobalMode
				? path.join(baseDir, `rules-${slug}`)
				: path.join(baseDir, ".ai-code-orchestrator", `rules-${slug}`)
	...
	private async importRulesFiles(
		...
		if (source === "global") {
			...
			const workspacePath = getWorkspacePath()
			baseDir = path.join(workspacePath, ".ai-code-orchestrator")
			rulesFolderPath = path.join(baseDir, `rules-${importMode.slug}`)
		}
