...
describe("CustomModesManager", () => {
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .agent-modes file and JSON for global customModes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]

			const agentModes = [{ slug: "mode2", name: "Mode 2", roleDefinition: "Role 2", groups: ["read"] }]
			...
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				...
				if (path === mockAgentModes) {
					return yaml.stringify({ customModes: agentModes })
				}
				throw new Error("File not found")
			})
		...
		it("should merge modes with .agent-modes taking precedence", async () => {
			const settingsModes = [
			...
			// Should contain 3 modes (mode1 from settings, mode2 and mode3 from agent-modes)
			expect(modes).toHaveLength(3)
			expect(modes.map((m) => m.slug)).toEqual(["mode2", "mode3", "mode1"])

			// mode2 should come from .agent-modes since it takes precedence
		...
		it("should handle missing .agent-modes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .agent-modes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .agent-modes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .agent-modes precedence", async () => {
			...
			let agentModesContent = { customModes: agentModes }
			...
			// Should update global state with merged modes where .agent-modes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "AgentModes Mode 1", // .agent-modes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .agent-modes file when adding project-specific mode", async () => {
			...
			// Mock .agent-modes to not exist initially
			...
			// Verify .agent-modes was created with the project mode
			...
			// Verify the content written to .agent-modes
	...
	describe("updateModesInFile", () => {
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".agent-modes"),
			...
			it("should handle import errors gracefully", async () => {
				...
				// Mock fs.writeFile to work normally for .agent-modes but we won't get there
				;(fs.writeFile as Mock).mockResolvedValue(undefined)
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				// Verify that no files were written outside the .ai-code-orchestrator directory
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".agent-modes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(path.join(mockWorkspacePath, ".ai-code-orchestrator"))
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(
					expect.stringContaining(path.join(".ai-code-orchestrator", "rules-test-mode")),
					{
						recursive: true,
						force: true,
					},
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .agent-modes file", async () => {
		...
		it("should return false when .agent-modes doesn't exist and mode is not a custom mode", async () => {
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
			...
			;(fs.readdir as Mock).mockResolvedValue([
				{
					name: "rule1.md",
					isFile: () => true,
					parentPath: "/mock/workspace/.ai-code-orchestrator/rules-test-mode",
				},
			])
		...
		it("should work with global custom modes when .agent-modes doesn't exist", async () => {
			...
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath // .agent-modes doesn't exist
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a custom mode in .agent-modes", async () => {
			const agentModesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .agent-modes", async () => {
			const agentModesContent = {
				customModes: [
					{
						slug: "code",
			...
			;(fs.readdir as Mock).mockResolvedValue([
				{
					name: "rule1.md",
					isFile: () => true,
					parentPath: "/mock/workspace/.ai-code-orchestrator/rules-code",
				},
			])
		...
		it("should successfully export global mode with rules from global .ai-code-orchestrator directory", async () => {
			// Mock a global mode
			const globalMode = {
