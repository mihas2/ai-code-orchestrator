...
import type { ProviderName } from "@ai-code-orchestrator/types"
	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			;(vscode.window.showSaveDialog as Mock).mockResolvedValue({
				fsPath: "/mock/path/ai-code-orchestrator-settings.json",
			})
			...
			expect(safeWriteJson).toHaveBeenCalledWith("/mock/path/ai-code-orchestrator-settings.json", {
		...
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(
				path.join("/mock/home", "Downloads", "ai-code-orchestrator-settings.json"),
			)
		})
