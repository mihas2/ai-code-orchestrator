...
/**
 * Controls write access to AI Code Orchestrator configuration files by enforcing protection patterns.
 * Prevents auto-approved modifications to sensitive AI Code Orchestrator configuration files.
 */
export class AicoProtectedController {
	private cwd: string
	private ignoreInstance: Ignore

	// Predefined list of protected AI Code Orchestrator configuration patterns
	...
	constructor(cwd: string) {
		...
		this.ignoreInstance.add(AicoProtectedController.PROTECTED_PATTERNS)
	}
	...
	getProtectionMessage(): string {
		return "This is a AI Code Orchestrator configuration file and requires approval for modifications"
	}
	...
	getInstructions(): string {
		const patterns = AicoProtectedController.PROTECTED_PATTERNS.join(", ")
		return `# Protected Files\n\n(The following AI Code Orchestrator configuration file patterns are write-protected and always require approval for modifications, regardless of autoapproval settings. When using list_files, you'll notice a ${SHIELD_SYMBOL} next to files that are write-protected.)\n\nProtected patterns: ${patterns}`
	}
	...
	static getProtectedPatterns(): readonly string[] {
		return AicoProtectedController.PROTECTED_PATTERNS
	}
}
