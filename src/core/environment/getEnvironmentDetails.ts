...
import type { ExperimentId } from "@ai-code-orchestrator/types"
...
export async function getEnvironmentDetails(cline: Task, includeFileDetails: boolean = false) {
	...
	// Filter paths through aicoIgnoreController
	const allowedVisibleFiles = cline.aicoIgnoreController
		? cline.aicoIgnoreController.filterPaths(visibleFilePaths)
	...
	// Filter paths through aicoIgnoreController
	const allowedOpenTabs = cline.aicoIgnoreController
		? cline.aicoIgnoreController.filterPaths(openTabPaths)
	...
	if (includeFileDetails) {
		...
		if (isDesktop) {
			...
			if (maxFiles === 0) {
				...
				const { showAicoIgnoredFiles = false } = state ?? {}

				const result = formatResponse.formatFilesList(
					cline.cwd,
					files,
					didHitLimit,
					cline.aicoIgnoreController,
