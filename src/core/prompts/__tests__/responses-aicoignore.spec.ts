// npx vitest core/prompts/__tests__/responses-aicoignore.spec.ts
...
describe("AicoIgnore Response Formatting", () => {
	...
	describe("formatResponse.aicoIgnoreError", () => {
		/**
		 * Tests the error message format for ignored files
		 */
		it("should format error message for ignored files", () => {
			const errorMessage = formatResponse.aicoIgnoreError("secrets/api-keys.json")
			...
			expect(parsed.suggestion).toContain("update the .aicoignore file")
		})
		...
		it("should include the file path in the error message", () => {
			...
			for (const testPath of paths) {
				const errorMessage = formatResponse.aicoIgnoreError(testPath)
				const parsed = JSON.parse(errorMessage) as any
	...
	describe("formatResponse.formatFilesList with AicoIgnoreController", () => {
		/**
		 * Tests file listing with aicoignore controller
		 */
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller
			const controller = new AicoIgnoreController(TEST_CWD)
		...
		/**
		 * Tests formatFilesList when showAicoIgnoredFiles is set to false
		 */
		it("should hide ignored files when showAicoIgnoredFiles is false", async () => {
			// Create controller
			const controller = new AicoIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Format with controller and showAicoIgnoredFiles = false
			const result = formatResponse.formatFilesList(
				TEST_CWD,
				files,
				false,
				controller as any,
				false, // showAicoIgnoredFiles = false
			)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with AicoIgnoreController
		 */
		it("should handle truncation with AicoIgnoreController", async () => {
			// Create controller
			const controller = new AicoIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with AicoIgnoreController", async () => {
			// Create controller
			const controller = new AicoIgnoreController(TEST_CWD)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .aicoignore instructions for the LLM", async () => {
			// Create controller
			const controller = new AicoIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .aicoignore exists", async () => {
			// Set up no .aicoignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .aicoignore
			const controller = new AicoIgnoreController(TEST_CWD)
			await controller.initialize()
