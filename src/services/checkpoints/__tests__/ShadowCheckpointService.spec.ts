...
const initWorkspaceRepo = async ({
	workspaceDir,
	userName = "AI Code Orchestrator",
	userEmail = "support@AI Code Orchestrator",
	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#hasNestedGitRepositories`, () => {
			it("throws error when nested git repositories are detected during initialization", async () => {
				...
				await mainGit.init()
				await mainGit.addConfig("user.name", "AI Code Orchestrator")
				await mainGit.addConfig("user.email", "support@AI Code Orchestrator")
				...
				await nestedGit.addConfig("user.name", "AI Code Orchestrator")
				await nestedGit.addConfig("user.email", "support@AI Code Orchestrator")
		...
		describe(`${klass.name}#saveCheckpoint with allowEmpty option`, () => {
			...
			it("isolates checkpoint operations from GIT_DIR environment variable", async () => {
				...
				// In the real-world Dev Container scenario, GIT_DIR is set BEFORE AI Code Orchestrator starts,
