...
// Global aico directory - computed once
const GLOBAL_AICO_DIR = p(HOME_DIR, ".ai-code-orchestrator")
const GLOBAL_AGENTS_DIR = p(HOME_DIR, ".agents")

// Mock aico-config
vi.mock("../../aico-config", () => ({
	getGlobalAicoDirectory: () => GLOBAL_AICO_DIR,
...
describe("SkillsManager", () => {
	...
	const globalSkillsDir = p(GLOBAL_AICO_DIR, "skills")
	const globalSkillsCodeDir = p(GLOBAL_AICO_DIR, "skills-code")
	const globalSkillsArchitectDir = p(GLOBAL_AICO_DIR, "skills-architect")
	const projectAicoDir = p(PROJECT_DIR, ".ai-code-orchestrator")
	const projectSkillsDir = p(projectAicoDir, "skills")
...
# Too Long Description`)
		...
		it("should handle symlinked skills directory", async () => {
			...
			// Simulate .ai-code-orchestrator/skills being a symlink to /shared/skills
			mockDirectoryExists.mockImplementation(async (dir: string) => {
				return dir === globalSkillsDir
			})
...
Instructions here...`
		...
		it("should handle symlinked skill subdirectory", async () => {
			...
			// Simulate .ai-code-orchestrator/skills/my-alias being a symlink to /external/actual-skill
			mockDirectoryExists.mockImplementation(async (dir: string) => {
				return dir === globalSkillsDir
			})
...
Instructions here...`
		...
		it("should prioritize .ai-code-orchestrator skills over .agents skills with same name", async () => {
			const agentSkillDir = p(globalAgentsSkillsDir, "common-skill")
			const agentSkillMd = p(agentSkillDir, "SKILL.md")
			const aicoSkillDir = p(globalSkillsDir, "common-skill")
			const aicoSkillMd = p(aicoSkillDir, "SKILL.md")
			...
			mockStat.mockImplementation(async (pathArg: string) => {
				if (pathArg === agentSkillDir || pathArg === aicoSkillDir) {
					return { isDirectory: () => true }
				}
				throw new Error("Not found")
			})
			...
			mockFileExists.mockImplementation(async (file: string) => {
				return file === agentSkillMd || file === aicoSkillMd
			})
...
# Agent Common Skill`
				...
				if (file === aicoSkillMd) {
					return `---
name: common-skill
description: AI Code Orchestrator version (should take priority)
---

# AI Code Orchestrator Common Skill`
				}
				throw new Error("File not found")
			})
			...
			// .ai-code-orchestrator should override .agents
			expect(commonSkill?.description).toBe("AI Code Orchestrator version (should take priority)")
		})
...
Instructions`)
	...
	describe("createSkill", () => {
		it("should create a new global skill", async () => {
			...
			expect(createdPath).toBe(p(GLOBAL_AICO_DIR, "skills", "new-skill", "SKILL.md"))
			expect(mockMkdir).toHaveBeenCalledWith(p(GLOBAL_AICO_DIR, "skills", "new-skill"), { recursive: true })
			expect(mockWriteFile).toHaveBeenCalled()

			// Verify the content written
			const writeCall = mockWriteFile.mock.calls[0]
			expect(writeCall[0]).toBe(p(GLOBAL_AICO_DIR, "skills", "new-skill", "SKILL.md"))
		...
		it("should create a mode-specific skill with modeSlugs array", async () => {
			...
			expect(createdPath).toBe(p(GLOBAL_AICO_DIR, "skills", "code-skill", "SKILL.md"))
		...
		it("should create a project skill", async () => {
			...
			expect(createdPath).toBe(p(PROJECT_DIR, ".ai-code-orchestrator", "skills", "project-skill", "SKILL.md"))
		})
...
Instructions`)
	...
	describe("moveSkill", () => {
		it("should move a skill from generic to mode-specific directory", async () => {
			...
			const destDir = p(GLOBAL_AICO_DIR, "skills-code", "test-skill")
			const destSkillsDir = p(GLOBAL_AICO_DIR, "skills-code")
...
Instructions`)
		...
		it("should move a skill from one mode to another", async () => {
			const sourceSkillsDir = p(GLOBAL_AICO_DIR, "skills-code")
			const sourceDir = p(sourceSkillsDir, "test-skill")
			const testSkillMd = p(sourceDir, "SKILL.md")
			const destDir = p(GLOBAL_AICO_DIR, "skills-architect", "test-skill")
			const destSkillsDir = p(GLOBAL_AICO_DIR, "skills-architect")
