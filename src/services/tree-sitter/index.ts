...
export async function parseSourceCodeDefinitionsForFile(
	...
	if (ext === ".md" || ext === ".markdown") {
		...
		if (aicoIgnoreController && !aicoIgnoreController.validateAccess(filePath)) {
			return undefined
		}
	...
	const definitions = await parseFile(filePath, languageParsers, aicoIgnoreController)
	if (definitions) {
		return `# ${path.basename(filePath)}\n${definitions}`
	}
 ...
 * @param aicoIgnoreController - Optional controller to check file access permissions
