...
function asObjectSafe(value: any): object {
	...
	try {
		...
		console.warn("AI Code Orchestrator <Language Model API>: Failed to parse object:", error)
		return {}
	}
}
			...
			if (item instanceof vscode.LanguageModelToolCallPart) {
				...
				if (item.input && Object.keys(item.input).length > 0) {
					try {
						...
						console.error(
							"AI Code Orchestrator <Language Model API>: Failed to stringify tool call input:",
							error,
						)
					}
				}
