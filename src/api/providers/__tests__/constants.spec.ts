...
describe("DEFAULT_HEADERS", () => {
	...
	it("should have correct HTTP-Referer value", () => {
		expect(DEFAULT_HEADERS["HTTP-Referer"]).toBe("https://github.com/AicoVetGit/AI Code Orchestrator-Cline")
	})

	it("should have correct X-Title value", () => {
		expect(DEFAULT_HEADERS["X-Title"]).toBe("AI Code Orchestrator")
	})

	it("should have correct User-Agent format", () => {
		const userAgent = DEFAULT_HEADERS["User-Agent"]
		expect(userAgent).toBe(`AiCodeOrchestrator/${Package.version}`)
	...
	it("should have User-Agent with correct tool name", () => {
		const userAgent = DEFAULT_HEADERS["User-Agent"]
		expect(userAgent.startsWith("AiCodeOrchestrator/")).toBe(true)
	})
