...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("ai-code-orchestrator", context)
			...
			setRuntimeConfig("ai-code-orchestrator", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("ai-code-orchestrator", "testSetting", "testValue")

			expect(getRuntimeConfig("ai-code-orchestrator.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("ai-code-orchestrator", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("ai-code-orchestrator.setting1")).toBe("value1")
			expect(getRuntimeConfig("ai-code-orchestrator.setting2")).toBe(42)
			expect(getRuntimeConfig("ai-code-orchestrator.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("ai-code-orchestrator.defined")).toBe("value")
			expect(getRuntimeConfig("ai-code-orchestrator.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("ai-code-orchestrator", "setting1", "value1")
			setRuntimeConfig("ai-code-orchestrator", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("ai-code-orchestrator.setting1")).toBeUndefined()
			expect(getRuntimeConfig("ai-code-orchestrator.setting2")).toBeUndefined()
		})
